...
static enum hrtimer_restart FunctionTimerTX(struct hrtimer *t) {
    ...
    if (strlen(TX_BUFFER) > 0 && GPIO_TX) //Data ready to send
    {
        if (bit == -1) {//Start bit 
...
static int __init ModuleInit(void) {
    ...
    gpio_request(GPIO_TX, "softuart gpio tx");
    gpio_direction_output(GPIO_TX, 1);
    gpio_request(GPIO_RX, "softuart gpio rx");
